...
function createAdmissionDates() {
    const container = document.getElementById('adm-dates-container');
    const ayLabel = document.getElementById('adm-ay');
    if (!container) return;

    fetch('data/admission.json')
        .then(response => response.json())
        .then(data => {
            const { title, academicYear, dates } = data.admissionDates;

            // Set academic year label
            if (ayLabel) ayLabel.textContent = academicYear;

            // Create HTML for each date item
            const createDateItem = (item) => `
                <div class="di">
                    <div class="dl">${item.label}</div>
                    <div class="dv">${item.date}</div>
                    <div class="dn">${item.note}</div>
                </div>
            `;

            // Render all date items
            container.innerHTML = dates.map(createDateItem).join('');
        })
...
function createCourses() {
    ...
    <div class="cc-bottom">
      ...
      <button class="cc-btn" onclick="openCourseModal(${course.id})">
        View Details →
      </button>
    </div>
`;
    ...
    <div class="modal-content">
        ...
        <h2>
          <i class="${course.icon}"></i> ${course.name}
        </h2>
